...
namespace PhysicsEngine
{
	...
	void PxInit()
	{
		...
		if (!default_material)
			default_material = physics->createMaterial(0.0f, 0.0f, 0.2f);

		if (!metal)
			metal = physics->createMaterial(0.2f, 0.0f, 0.0f);

		/*if (!wood_material)
			wood_material = physics->createMaterial(0.375f, 0.375f, 0.0f);*/
	}
	...
	PxRigidDynamic* Scene::GetRightFlipper()
	{
		return rightFlipper_actor;
	}

	PxRigidDynamic* Scene::GetLeftFlipper()
	{
		return leftFlipper_actor;
	}

	PxRigidDynamic* Scene::GetSpringPlunger()
	{
		return springPlunger_actor;
	}
	...
	void Scene::SelectNextActor()
	{
		...
		if(nbActors)
		{
			...
			rightFlipper_actor = actors[1];
			leftFlipper_actor = actors[2];
			springPlunger_actor = actors[8];
			//lid = actors[11];
			if (selected_actor)
			{
