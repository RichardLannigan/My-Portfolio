...
namespace PhysicsEngine
{
	...
	ostringstream convertLives;
	ostringstream convertScore;
	ostringstream convertMultiplier;
	ostringstream convertTotalScore;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		Renderer::InitWindow(window_name, 1920, 1080);
		//Renderer::InitWindow(window_name, 800, 650);
		Renderer::Init();

		camera = new Camera(PxVec3(-60.0f, 65.0f, 20.0f), PxVec3(0.f,-0.75,-1.f), 5.f);

		//initialise HUD
		HUDInit();

		///Assign callbacks and render
	...
	void HUDInit()
	{
		if (timer == 59) //Re-displays every second appose to every tick to increase effeiciency.
		{
			//Uses string stream to convert variables to strings.
			convertLives.str("");
			convertLives.clear();
			convertLives << lives;
			livesString = convertLives.str();

			convertScore.str("");
			convertScore.clear();
			convertScore << roundScore;
			scoreString = convertScore.str();

			convertMultiplier.str("");
			convertMultiplier.clear();
			convertMultiplier << scoreMultiplier;
			multiplierString = convertMultiplier.str();

			convertTotalScore.str("");
			convertTotalScore.clear();
			convertTotalScore << totalScore;
			totalScoreString = convertTotalScore.str();

			//Create and handle HUD
			hud.Clear(1); //Refresh HUD so text doesn't build up.
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "Controls");
			hud.AddLine(HELP, "   -Press the 'F' and 'H' keys to control the flippers.");
			hud.AddLine(HELP, "   -Hold down the 'G' key to retract the plunger ");
			hud.AddLine(HELP, "    and release to fire the pinball.");
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "Extra controls");
			hud.AddLine(HELP, "   -F10 - pause/resume");
			hud.AddLine(HELP, "   -F5 - help on/off");
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "   You have three tries until game over.");
			hud.AddLine(HELP, "   Good luck!!!");
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "   Lives: " + livesString);
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "   Round Score: " + scoreString);
			hud.AddLine(HELP, "");
			hud.AddLine(HELP, "   Round Multiplier: " + multiplierString);
			if (lives == 0)
			{
				hud.AddLine(HELP, "   Lives: 0");
				hud.AddLine(HELP, "");
				hud.AddLine(HELP, "                                                                                              GAME OVER");
				hud.AddLine(HELP, "");
				hud.AddLine(HELP, "                                                                                        Total Score: " + totalScoreString);
				//scene->Pause(!scene->Pause());
			}
			hud.AddNumber(HELP, lives);
		}
	}
	...
	void ForceInput(int key)
	{
		...
		case 'M': //down
			...
			//----Pinball machine controls----//
		case 'F'://Manipulate right flipper
			scene->GetLeftFlipper()->setAngularVelocity(PxVec3(0.125,1,0)*15,true);
			break;
		case 'H'://Manipulate left flipper
			scene->GetRightFlipper()->setAngularVelocity(PxVec3(0.125,1,0)*-15,true);
			break;
		case 'G'://Manipulate plunger
			scene->GetSpringPlunger()->addForce(PxVec3(0,0,1)*600);
			break;
	...
	void KeySpecial(int key, int x, int y)
	{
		...
		switch (key)
		{
		case GLUT_KEY_LEFT://Manipulate right flipper
				scene->GetLeftFlipper()->setAngularVelocity(PxVec3(0.125,1,0)*15,true);
			break;
			//display control
		case GLUT_KEY_F5:
