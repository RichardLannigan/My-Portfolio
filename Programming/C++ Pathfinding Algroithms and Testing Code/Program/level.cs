...
namespace Pathfinder
{
    ...
    class Level
    {
        public int gridSize = 40; //set the map grid size
        public int [,] tiles; //a 2d array of 0's and 1's: 0 = free cell, 1 = blocked cell
        public int obstacles = 125;
        int obstacleChance = 95;
        Random rand = new Random();
        public int gridBorder = 0;
        ...
        public void BuildLevel(Player player, Bot bot) //Randomly generates grid of specified size and obstacle complexity.
        {
            while (obstacles != 0)
            {
                for (int i = 0; i < gridSize; i++)
                {
                    for (int j = 0; j < gridSize; j++)
                    {
                        if (i == 0 || j == 0 || i == gridBorder || j == gridBorder) //Draws a border around the map.
                            tiles[i, j] = 1;
                        else
                        {
                            if (rand.Next(0, 100) > obstacleChance && obstacles > 0)
                            {
                                tiles[i, j] = 1;
                                obstacles--;
                            }
            ...
            //Ensures that there are no obstacles at the player's and the bot's position.
            tiles[player.GridPosition.X, player.GridPosition.Y] = 0;
            tiles[bot.GridPosition.X, bot.GridPosition.Y] = 0;
        }
